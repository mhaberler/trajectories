#!/usr/bin/env python3
"""
Call GET /v1/wind with times= (launch-window batch) and print flat JSON.
...
Local uvicorn instead:
  ...
  TRAJECTORIES_API_URL=http://127.0.0.1:8010 python python/examples/api_point_wind_times.py
...
OUT = Path(__file__).resolve().parent / "out_api_point_wind_times.json"
...
def main() -> int:
    ...
    print(f"# {len(samples)} sample(s)", file=sys.stderr)
    for sample in samples:
        models = sample.get("models") or []
        ok = sum(1 for m in models if not m.get("error"))
        print(
            f"# {sample.get('time')}: {ok}/{len(models)} model(s) ok",
            file=sys.stderr,
        )
